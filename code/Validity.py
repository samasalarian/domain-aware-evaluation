#!/usr/bin/env python3
"""
Experiment 4: Biological Naturalness of Evo2-Generated Genomes
...
# Output path
OUTPUT_DIR = Path("results/Validity/outputs")
...
print("=" * 80)
print("Experiment 4: Biological Naturalness Analysis (REVISED)")
print("=" * 80)
...
# Plot 1: Violin plot - ORF Length (Aggregated)
...
ax.set_ylabel('ORF Length (amino acids)', fontsize=12, fontweight='bold')
ax.set_title('Distribution of ORF Lengths: Natural vs Evo2-Generated Genomes', 
             fontsize=14, fontweight='bold', pad=20)
ax.set_xticklabels(['Natural', 'Synthetic Genome (40B)', 'Synthetic Genome (7B)'])
plt.tight_layout()

violin_file = OUTPUT_DIR / "orf_length_violin.png"
plt.savefig(violin_file, dpi=300, bbox_inches='tight')
plt.savefig(OUTPUT_DIR / "orf_length_violin.pdf", bbox_inches='tight')
print(f"  Saved: {violin_file}")
plt.close()

# Plot 2: Density overlay - ORF Length
fig, ax = plt.subplots(figsize=(10, 6))
...
plt.savefig(OUTPUT_DIR / "orf_length_density.pdf", bbox_inches='tight')
print(f"  Saved: {density_file}")
plt.close()

# Plot 3: ECDF - ORF Length (NEW)
fig, ax = plt.subplots(figsize=(10, 6))
...
plt.savefig(OUTPUT_DIR / "orf_length_ecdf.pdf", bbox_inches='tight')
print(f"  Saved: {ecdf_file}")
plt.close()

# Plot 4: Genome-level scatter (NEW)
fig, ax = plt.subplots(figsize=(10, 8))

# Plot each genome
for idx, row in genome_level_stats.iterrows():
    if row['genome_type'] == 'Natural':
        ax.scatter(row['median_orf_length'], row['pct_no_hit'], 
                  s=300, color=COLORS['Natural'], marker='*', 
                  edgecolor='black', linewidth=2, zorder=10,
                  label='Natural' if idx == 0 else '')
    else:
...
ax.set_xlabel('Median ORF Length (amino acids)', fontsize=12, fontweight='bold')
ax.set_ylabel('% Proteins with No Pfam Hit', fontsize=12, fontweight='bold')
ax.set_title('Genome-Level: ORF Length vs Pfam Annotation Completeness', 
             fontsize=14, fontweight='bold', pad=20)
ax.legend(loc='best', fontsize=11)
ax.grid(True, alpha=0.3)
plt.tight_layout()

scatter_file = OUTPUT_DIR / "genome_level_scatter.png"
plt.savefig(scatter_file, dpi=300, bbox_inches='tight')
plt.savefig(OUTPUT_DIR / "genome_level_scatter.pdf", bbox_inches='tight')
print(f"  Saved: {scatter_file}")
plt.close()

# Plot 5: Bar plot - % No-Hit
fig, ax = plt.subplots(figsize=(10, 6))
...
bars = ax.bar(bar_data['model'], bar_data['mean'], yerr=bar_data['std'], 
              color=colors_bar, alpha=0.7, capsize=5, edgecolor='black', linewidth=1.5)

ax.set_xlabel('Model', fontsize=12, fontweight='bold')
ax.set_ylabel('% Proteins with No Pfam Hit', fontsize=12, fontweight='bold')
ax.set_title('Pfam Annotation Completeness: Natural vs Evo2-Generated Genomes', 
             fontsize=14, fontweight='bold', pad=20)
ax.set_xticklabels(['Natural', 'Synthetic (40B)', 'Synthetic (7B)'])
plt.tight_layout()

bar_file = OUTPUT_DIR / "pfam_no_hit_barplot.png"
plt.savefig(bar_file, dpi=300, bbox_inches='tight')
plt.savefig(OUTPUT_DIR / "pfam_no_hit_barplot.pdf", bbox_inches='tight')
print(f"  Saved: {bar_file}")
plt.close()

# Plot 6: Combined figure (3 panels)
...
plt.tight_layout()
combined_file = OUTPUT_DIR / "experiment4_combined_figure.png"
plt.savefig(combined_file, dpi=300, bbox_inches='tight')
plt.savefig(OUTPUT_DIR / "experiment4_combined_figure.pdf", bbox_inches='tight')
